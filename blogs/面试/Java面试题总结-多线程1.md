...
wait(): Object 类中定义的实例方法。对象调用 wait 方法会让当前线程进入等待状态（前提是当前线程持有该对象），此时当前线程会释放相应对象，这样一来其它线程便有机会获取这个对象。当其它线程获取了这个对象并进行了所需操作时，便可以调用 notify 方法唤醒之前进入等待状态的线程。

sleep(): Thread 类中的静态方法，作用是让当前线程进入休眠状态，以便让其他线程有机会执行。进入休眠状态的线程不会释放它所持有的锁。


## 当一个线程进入一个对象的synchronized修饰的方法A之后，其它线程是否可进入此对象的synchronized方法B？
...
## 编写多线程程序有几种实现方式？启动一个线程是调用 run()还是 start()方法？
三种方式：
...
线程池顾名思义就是事先创建若干个可执行的线程放入一个池（容器）中，需要的时候从池中获取线程，从而不用自行创建线程，使用完毕不需要销毁线程而是放回池中，从而减少创建和销毁线程对象的开销。

## 为什么要用线程池

1. 减少了创建和销毁线程的次数，每个工作线程都可以被重复利用，可执行多个任务。
2. 可以根据系统的承受能力，调整线程池中工作线线程的数目，防止因为过多的线程，从而影响了服务器的运行速度。线程不是越多越好的。
