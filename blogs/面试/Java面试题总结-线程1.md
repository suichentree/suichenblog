...
- 线程安全 - 如果多个线程在执行过程中不会产生共享资源上的冲突，则线程安全。
- 线程不安全 - 如果有多个线程在执行过程中会产生共享资源上的冲突，则线程不安全。


### 线程不安全的原理是什么？

java的内存模型中有主内存和线程的工作内存之分。主内存上存放的是线程共享的数据，线程的工作内存是线程私有的空间，存放的是线程私有的数据。

如果线程要操作共享数据，那么线程并不是直接去修改共享数据而是会先在线程私有的工作内存中创建一份数据，然后刷到主内存中去。

线程中主内存和工作内存之间的关系
![java_20231129161419.png](../blog_img/java_20231129161419.png)

如果有多个线程同时操作主内存的数据，由于线程本身是抢占cpu执行的机制就会带来冲突的问题，也就是多线程的安全问题。

线程安全的定义就是：如果多个线程并发执行过程中不会产生共享资源的冲突就是线程安全的。
