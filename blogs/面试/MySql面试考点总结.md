...
介绍B+树索引，就不得不提二叉树，平衡二叉树和B树这三种数据结构。B+树就是从它们演化来的。

#### 二叉树

![20221107175851.png](../blog_img/20221107175851.png)

二叉树中存储了键(key)和数据(data)。

二叉树的特点就是任何节点的左子节点的键值都小于当前节点的键值，右子节点的键值都大于当前节点的键值。顶端的节点我们称为根节点，没有子节点的节点我们称之为叶节点。

若二叉树出现这样的构造：
![20221108144628.png](../blog_img/20221108144628.png)

对于上面的二叉树，若查找id=17的用户信息，需要查找7次，也就相当于全表扫描了。导致这个现象的原因其实是二叉树变得不平衡了，也就是高度太高了，从而导致查找效率的不稳定。

因此为了需要二叉树一直保持平衡。出现了平衡二叉树。

#### 平衡二叉树

平衡二叉树又称AVL树，在二叉树的基础上，要求每个节点的左右子树的高度差不能超过1。
