...
- ZSet 类型：有序集合，自动去重。可以存储想要排序的集合数据。例如各个药店的每日销售额数据，形成一个排行榜。
...
- 修改频率很高，但是不得不缓存的数据。例如点赞数，收藏数，分享数等是非常典型的热点数据。但是又不断变化，此时就不得不将数据同步保存到Redis缓存，减少数据库压力
...
3. volatile-ttl：最近过期算法,从设置了过期时间的键中选择过期时间最早的键值对清除。
4. volatile-random：随机选择算法，从设置了过期时间的键中，随机选择键进行清除。
...
Redis 集群目前无法做数据库选择，集群中的每个redis节点默认使用 0 数据库。
