...
1. 第一次同步，主机生成RDB持久化文件给从机，并将同步期间的后续写操作存储到缓存中。
2. 然后从机执行完RDB文件后，再通知主机将缓存中的写操作命令，发送给从机。让从机执行一遍。
3. 最后同步过程就完成了。
...
简单比喻：微服务有多个用户服务节点，每个用户服务节点都有注册功能。注册功能会涉及到对redis数据库的读写操作。当多个线程同时执行多个用户服务节点的注册功能的时候，那么就会涉及到redis的多进程下的多线程读写操作。此时redis就需要分布式锁，来保证多线程读写操作的线程安全性。
