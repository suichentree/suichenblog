...
在 request 中，如果直接利用get()或post()等方法，去发送请求给目标服务器的时候。这些不同的请求是各自独立的，即相当于不同的会话，这些请求是无法共享cookie。。也就是说相当于你用两个浏览器打开了两个不同的页面一样。

解决这个问题的方式就是将两个请求，都放到一个会话之中。即同一个Session对象中。
...
requests 提供SSL证书验证的功能。当发送HTTP请求的时候，它会检查SSL证书，可以通过使用verify属性来控制是否检查SSL证书。
