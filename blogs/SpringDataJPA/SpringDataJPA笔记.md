...
Spring Data JPA，主要用于简化持久层代码的写法

## Spring Data JPA 介绍

> 什么是JPA？

<font color="red">JPA,又可以叫做JPA规范。</font>

JPA 全称 Java Persistence API, java 持久性 API。是sun 公司统一不同的ORM框架中的DAO层,Service层代码的规范。

其本质上就是一种对象映射（ORM）规范，注意JPA不是ORM框架，而是一种接口规范。

因为JPA只是制订了一些规范，提供了一些实现这种规范需要的API接口。因此JPA规范的具体实现则由各个服务器厂商来提供实现。

例如：JBoss应用服务器底层就以Hibernate作为JPA的实现。
...
当开发者实现一套JPA规范的代码后，代码底层的JPA实现可以任意切换：觉得Hibernate好的，可以选择Hibernate的JPA规范实现；觉得TopLink好的，可以选择TopLink的JPA规范实现……这样开发者可以避免为使用Hibernate学习一套ORM框架，为使用TopLink又要再学习一套ORM框架。
...
1. Spring Data JPA 是Spring公司在JPA规范的基础上提供了Repository 持久层的实现。
2. 不同的ORM框架实现JPA规范的代码还是有一些差异的。如果我们要更换ORM框架，那么还是需要更改一些代码的。而<font color="red">Spring Data Jpa能够方便大家在不同的ORM框架中间进行切换而不要更改代码。并且Spring Data Jpa 对 Repository层封装的很好，可以省去不少的麻烦。</font>
